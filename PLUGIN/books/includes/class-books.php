...
class Books {
	...
	private function load_dependencies() {
		 ...
		 * core plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-books-loader.php';

		/**
		 * The class responsible for defining internationalization functionality
		 * of the plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-books-i18n.php';

		/**
		 * The class responsible for defining all actions that occur in the admin area.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-books-admin.php';

		/**
		 * The class responsible for defining all actions that occur in the public-facing
		 * side of the site.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-books-public.php';
	...
	private function define_admin_hooks() {
        ...
        $this->loader->add_action( 'init', $plugin_admin, 'post_type_book' );
        ...
        $this->loader->add_action( 'init', $plugin_admin, 'taxonomy_non_book_tag' );

        // Hook for registering function to add custom Meta Box
        $this->loader->add_action( 'add_meta_boxes', $plugin_admin, 'book_add_meta_box' );
        
        // Hook for saving meta information on every post save
        $this->loader->add_action( 'save_post', $plugin_admin, 'book_save_details_data' );
        
        // Hook for registering custom table to metadata api
        $this->loader->add_action( 'plugins_loaded', $plugin_admin, 'book_register_custom_table' );
        $this->loader->add_action( 'switch_blog', $plugin_admin, 'book_register_custom_table' );
        
        // Hook for custom admin options page
        $this->loader->add_action( 'admin_menu', $plugin_admin, 'book_details_register_options_page' );
        
        // Hook for registering settings
        $this->loader->add_action( 'admin_menu', $plugin_admin, 'book_details_register_settings' );
        
        // Hook for registering shortcode
        $this->loader->add_action( 'init', $plugin_admin, 'register_book_shortcode' );

        // Hook for registering widgets
        $this->loader->add_action( 'widgets_init', $plugin_admin, 'my_register_book_widget' );

        // Hook for registering custom dashboard widget
        $this->loader->add_action( 'wp_dashboard_setup', $plugin_admin, 'book_dashboard_widget' );
	}
